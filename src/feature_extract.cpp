...
void horizon_handler(const livox_ros_driver::CustomMsg::ConstPtr &msg)
{
  ...
  for(uint i=1; i<plsize; i++)
  {
        ...
        && (!IS_VALID(msg->points[i].x)) && (!IS_VALID(msg->points[i].y)) && (!IS_VALID(msg->points[i].z)))
    {
      ...
      if((std::abs(pl_full[i].x - pl_full[i-1].x) > 1e-8) 
          || (std::abs(pl_full[i].y - pl_full[i-1].y) > 1e-8)
          || (std::abs(pl_full[i].z - pl_full[i-1].z) > 1e-8))
      {
  ...
  {
    total_points += pl_buff[j].size();
    // ROS_WARN(" vaild points is too less ,only %d . ",  total_points );
  }

  // 有效的点很少，就用原始的点，每100个取一个
  if (total_points < 300)
  {
    for (uint i = 0; i < plsize; i = i + 200)
    {
      PointType ap;
      ap.x = msg->points[i].x;
      ap.y = msg->points[i].y;
      ...
      ap.curvature = msg->points[i].offset_time / float(1000000);
      pl_surf.push_back(ap);
    }
    // ROS_WARN(" vaild points is too less ,only %d . use %d original points",  total_points,  pl_surf.points.size());
    pub_func(pl_surf, pub_surf, msg->header.stamp);
    return;
  }
...
void velo16_handler(const sensor_msgs::PointCloud2::ConstPtr &msg)
{
  ...
  for(uint i=0; i<plsize; i++)
  {
    ...
    ap.curvature = ( pl_orig.points[i].timestamp - msg->header.stamp.toSec() ) * 1000 ;
...
void pub_func(pcl::PointCloud<PointType> &pl, ros::Publisher pub, const ros::Time &ct)
{
  // ROS_WARN(" pc s %ld ", pl.size());
  // if ( pl.size() > 300)
  // {
  //   static pcl::RadiusOutlierRemoval< PointType > outrem;
	//   outrem.setInputCloud( pl.makeShared() );
